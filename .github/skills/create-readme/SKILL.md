...
3. Classify the repository archetype and start from [`templates/readme/zsh-plugin.md`](../../../templates/readme/zsh-plugin.md) as the canonical structure:
   ...
   - **Zi Annexes (`z-a-*`):** Keep Zi as the installation path, and adapt the Lifecycle and Portable shell contract sections to document registered ice modifiers, annex hooks (`before_load`, `after_load`), owned state parameters, and the unload function.
   - **Compiled / Go Tools (`zsh-lint`):** Adapt plugin-specific sections into tool-specific installation, binary or module distribution, CLI flags, verification, and generated API reference coverage.
...
5. Lead Zsh-plugin installation guidance with Zi. Keep other manager examples concise and place them inside collapsible `<details>` disclosure blocks.
