...
1. **Blocking above a threshold.** Rejected for now: without an A/A history, hosted-runner noise produces false timing failures on unrelated pull requests, and every one of them would have to be waved through by the maintainer bypass that ADR-0013 keeps available for deadlocks, turning an escape route into routine practice. Revisit once the control column has a track record.
...
- [z-shell/zi#553](https://github.com/z-shell/zi/issues/553): the zi suite; [z-shell/zi#554](https://github.com/z-shell/zi/issues/554): the measured ice-tokenizing regression; [z-shell/zi#555](https://github.com/z-shell/zi/issues/555): the manifest-reader cost recorded as a baseline and optimization target.
