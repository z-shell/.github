...
5. **Publication.** Per-release results are committed by a maintainer under `benchmarks/results/<tag>/` after a release, together with the exact source revision, Zsh version, architecture, and runner or host identity. Automation uploads artifacts and job summaries; it never commits results. Results are compared only when those identity fields match.
6. **Pattern admission.** Once `z-shell/zi`'s suite lands, the shared shape (Zsh or Python runner, JSON report, `cases[case].results[variant]{median,p95,min,count}`, `change{median_delta_ms,median_delta_percent}`, `failure`) is recorded in `PATTERNS.md` as an observed two-repository pattern.
