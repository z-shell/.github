...
if pr.head.repo.full_name != pr.base.repo.full_name:
    if github_event_name != "pull_request_target":
        print(
            "ERROR: PRs from forks are only supported when trigger on "
            '"pull_request_target"',
            file=sys.stderr,
        )
        sys.exit(1)
...
if pr_invalid_labels:
    print(
        "Error! This pull request contains the following invalid labels: "
        f"{pr_invalid_labels}",
        file=sys.stderr,
    )
...
if not pr_valid_labels:
    ...
    print(
        "This pull request contains the following valid labels: " f"{pr_valid_labels}"
    )
