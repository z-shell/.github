...
- **Commit trailers:** `Co-authored-by` crediting a real human, including the PR author crediting themselves, is fine. Never credit a bot, AI agent, or automation as a co-author. The same applies to pull-request, issue, comment, review, and handoff text: no generated-with footer or agent credit, whatever a runtime's default suggests. `z-shell/.github` and `z-shell/zi` enforce the trailer rule in CI. Other repositories remain author-enforced until their own verified caller is live; do not infer enforcement from organization policy alone.
